...
def run_pitch_analysis(youtube_url: str) -> PitchAnalysisState:
    ...
    try:
        ...
        # Create error state from initial_state instead of undefined final_state
        error_state = initial_state.copy() if hasattr(initial_state, 'copy') else dict(initial_state)
        error_state["current_agent"] = "failed"
